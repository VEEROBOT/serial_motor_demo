...
class MotorDriver(Node):
    ...
    def send_pwm_motor_command(self, mot_1_pwm, mot_2_pwm, mot_3_pwm, mot_4_pwm):
        self.send_command(f"o {int(mot_1_pwm)} {int(mot_2_pwm)} {int(mot_3_pwm)} {int(mot_4_pwm)}")

    def send_feedback_motor_command(self, mot_1_ct_per_loop, mot_2_ct_per_loop, mot_3_ct_per_loop, mot_4_ct_per_loop):
        self.send_command(f"m {int(mot_1_ct_per_loop)} {int(mot_2_ct_per_loop)} {int(mot_3_ct_per_loop)} {int(mot_4_ct_per_loop)}")
    ...
    def motor_command_callback(self, motor_command):
        if (motor_command.is_pwm):
            self.send_pwm_motor_command(motor_command.mot_1_req_rad_sec, motor_command.mot_2_req_rad_sec, motor_command.mot_3_req_rad_sec, motor_command.mot_4_req_rad_sec)
        else:
            ...
            mot3_ct_per_loop = motor_command.mot_3_req_rad_sec * scaler
            mot4_ct_per_loop = motor_command.mot_4_req_rad_sec * scaler          
            self.send_feedback_motor_command(mot1_ct_per_loop, mot2_ct_per_loop, mot3_ct_per_loop, mot4_ct_per_loop)
