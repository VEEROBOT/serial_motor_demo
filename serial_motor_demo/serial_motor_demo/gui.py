...
class MotorGui(Node):
    ...
    def __init__(self):
        ...
        self.m3 = Scale(m3_frame, from_=-255, to=255, orient=HORIZONTAL)
    ...
    def send_motor_once(self):
        ...
        if (self.pwm_mode):
            ...
            msg.mot_4_req_rad_sec = float(self.m4.get())            
        else:
            msg.mot_1_req_rad_sec = float(self.m1.get()*2*math.pi)
            msg.mot_2_req_rad_sec = float(self.m2.get()*2*math.pi)
            msg.mot_3_req_rad_sec = float(self.m3.get()*2*math.pi)
            msg.mot_4_req_rad_sec = float(self.m4.get()*2*math.pi)            
